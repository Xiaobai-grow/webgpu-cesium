...
- [x] M4：Hillaire 四 LUT（2026-09-13）：透射 256×64、多散射 32×32（16 方向，非 64）、Sky-View 192×108、空气透视 32³、IBL 32²×6（非 64² + 5 级预滤波）。compute 在太阳方向变化或首帧跑，示例站 CPU ≈ 1 ms；集显逐 pass 成本未用 timestamp 测。
- [ ] M4：从地表到 400 km 轨道连续飞行时的能量与颜色连续性（本轮未做连续飞测）。
- [x] M4：曝光用相对单位（2026-09-13）：CPU `EnvironmentState` 按太阳高度在 0.45–0.85 平滑；`light.intensity` 乘辐照而非曝光。Bloom 阈值留给 M6。
- [x] M4：星表（2026-09-13）：约 30 颗亮星 + 程序星，远小于 9000 Hipparcos / 200 KB 目标；无 TAA，闪烁未测。
