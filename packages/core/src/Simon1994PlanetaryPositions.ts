// @ts-nocheck — 机械移植：算法与 Cesium 一致，严格类型后续补齐
/**
 * @license
 ...
 */
const Simon1994PlanetaryPositions = {} as {
...
function elementsToCartesian(
  ...
  if (
    type === "Hyperbolic" &&
    Math.abs(CesiumMath.negativePiToPi(trueAnomaly)) >= Math.acos(-1.0 / eccentricity)
  ) {
    throw new DeveloperError(
...
function computeSimonMoon(date, result) {
  ...
  const D =
    ...
    RadiansPerArcSecond * (1602961601.209 * t - 6.3706 * t2 + 0.006593 * t3 - 0.00003169 * t4)
  const F =
    93.27209062 * RadiansPerDegree +
    RadiansPerArcSecond * (1739527262.8478 * t - 12.7512 * t2 - 0.001037 * t3 + 0.00000417 * t4)
  const l =
    134.96340251 * RadiansPerDegree +
    RadiansPerArcSecond * (1717915923.2178 * t + 31.8792 * t2 + 0.051635 * t3 - 0.0002447 * t4)
  const lprime =
    357.52910918 * RadiansPerDegree +
    RadiansPerArcSecond * (129596581.0481 * t - 0.5532 * t2 + 0.000136 * t3 - 0.00001149 * t4)
  const psi =
    310.17137918 * RadiansPerDegree -
    RadiansPerArcSecond * (6967051.436 * t + 6.2068 * t2 + 0.007618 * t3 - 0.00003219 * t4)
